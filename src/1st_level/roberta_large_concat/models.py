...
class SelfAttention(torch.nn.Module):
    def __init__(self):
        ...
        self.linear1 = torch.nn.Linear(config.HIDDEN_SIZE*config.N_LAST_HIDDEN, config.ATTENTION_HIDDEN_SIZE*config.N_LAST_HIDDEN)          
        self.tanh = torch.nn.Tanh()            
        self.linear2 = torch.nn.Linear(config.ATTENTION_HIDDEN_SIZE*config.N_LAST_HIDDEN, 1)
...
class CommonlitModel(transformers.BertPreTrainedModel):
    def __init__(self, conf):
        ...
        self.classifier = torch.nn.Sequential(
            ...
            torch.nn.Linear(config.HIDDEN_SIZE*config.N_LAST_HIDDEN, config.HIDDEN_SIZE*config.N_LAST_HIDDEN),
            torch.nn.Dropout(config.CLASSIFIER_DROPOUT),
            torch.nn.Linear(config.HIDDEN_SIZE*config.N_LAST_HIDDEN, 1),
        )
